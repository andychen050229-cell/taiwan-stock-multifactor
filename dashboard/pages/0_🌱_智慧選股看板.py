...
@st.cache_data
def load_all_data():
    """載入所有需要的資料 - 優先從 dashboard/data/ 載入 (Streamlit Cloud)，否則從本地路徑 (開發環境)"""
    ...
    if companies is None or prices is None or income is None:
        try:
            ...
            # Load feature store (only available locally, used for market overview)
            fs = pd.read_parquet(base / "outputs" / "feature_store.parquet")
        except Exception as e:
            st.error(f"無法載入資料: {e}")
            st.stop()

    # Load Phase 2 report (available in both environments)
    ...
    return fs, companies, prices, income, report, recommendations
...
def get_recommendations(fs, companies, horizon=20, n_top=10, recommendations=None):
    ...
    優先使用預先計算的 recommendations (JSON)，否則從 feature store 計算。

    策略邏輯：
    - 如果有 recommendations JSON: 使用預先篩選的 UP 股票，合併公司名
    - 否則從 feature store 計算: 選出 label_{horizon} == 1 (UP) 的股票，按報酬排序
    """
    label_col = f"label_{horizon}"
    ret_col = f"fwd_ret_{horizon}"

    # Try using pre-computed recommendations first
    if recommendations:
        ...
        if horizon_key in recommendations:
            ...
            if stocks_list:
                ...
                # Ensure company_id is string for merge
                recs_df["company_id"] = recs_df["stock_id"].astype(str)

                # Merge with companies data for names
    ...
    # Fallback to feature store (local development)
...
# ===== Load Data =====
fs, companies, prices, income, report, recommendations = load_all_data()
