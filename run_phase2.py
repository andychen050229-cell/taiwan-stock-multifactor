...
def main():
    ...
    if not fs_path.exists():
        log.error(f"Feature Store not found: {fs_path}")
    ...
    log.info(f"  Memory: {fs.memory_usage(deep=True).sum() / 1024**2:.1f} MB")
    ...
    for h in horizons:
        ...
        if len(engines_available) >= 2:
            oof_list = [all_horizon_results[h][e]["oof_predictions"] for e in engines_available]
            ensemble_oof = np.nanmean(oof_list, axis=0)
            ensemble_labels = all_horizon_results[h][engines_available[0]]["oof_labels"]
            ...
            if valid_ens.sum() > 100:
                ...
                # 儲存 ensemble 到 all_horizon_results
                all_horizon_results[h]["ensemble"] = {
                    "engine": "ensemble",
                    "oof_predictions": ensemble_oof,
                    "oof_labels": ensemble_labels,
                    "avg_metrics": {
                        "auc": round(ens_auc, 4),
    ...
    for h in horizons:
        ...
        for eng in ["ensemble", "lightgbm", "xgboost"]:
            ...
            if "error" in res or res.get("oof_predictions") is None:
                continue
            up_prob = res["oof_predictions"][:, 2]
            test_idx = np.concatenate([f.test_idx for f in folds])
            qdf = fs.iloc[test_idx].copy()
    ...
    for eng in ["ensemble", "xgboost", "lightgbm"]:
        ...
        if not has_all:
            continue
        # 用 D+5 的 OOF predictions 測試不同 horizon 的 IC
        res5 = all_horizon_results[5].get(eng, {})
        if "error" in res5 or res5.get("oof_predictions") is None:
            continue
        up_prob = res5["oof_predictions"][:, 2]
        test_idx = np.concatenate([f.test_idx for f in folds])
        adf = fs.iloc[test_idx].copy()
        adf["_score"] = up_prob[test_idx]
        ret_cols = [f"fwd_ret_{h}" for h in horizons if f"fwd_ret_{h}" in fs.columns]
        if ret_cols:
            decay = compute_alpha_decay(adf, "_score", ret_cols, "trade_date")
            alpha_decay_results[eng] = decay["horizon_metrics"]
            log.info(f"  Alpha decay ({eng}): " + " | ".join(
                f"{k}: IC={v['mean_ic']:.4f}, ICIR={v['icir']:.3f}" for k, v in decay["horizon_metrics"].items()
            ))
        break
    ...
    try:
        ...
        for h in horizons:
            ...
            for eng, res in all_horizon_results[h].items():
                ...
                # Confusion matrix
                oof = res.get("oof_predictions")
                oof_labels = res.get("oof_labels")
                if oof is not None and oof_labels is not None:
                    oof_class = np.argmax(oof, axis=1).astype(float)
                    oof_class[np.isnan(oof_labels)] = np.nan
                    plot_confusion_matrix(oof_labels, oof_class, eng, h, fig_dir_str)
